...
class UploadService:
    ...
    async def process_csv_upload(self, file_content: bytes) -> Dict:
        ...
        # Parse CSV
        parsed_rows, unique_box_locations = self.csv_parser.parse_csv_content(file_content)
        ...
        # Step 1: Create or get boxes
        box_map = await self._create_or_get_boxes(unique_box_locations, stats)
    ...
    async def _create_or_get_boxes(
        ...
        Returns:
            Dict mapping box_location -> box_id
        """
        client = get_supabase_client()
        box_map = {}
        ...
        for location in box_locations:
            # Check if box exists
            response = client.table('boxes').select('id, name').eq('name', location).execute()

            if response.data and len(response.data) > 0:
                # Box exists
                box_id = UUID(response.data[0]['id'])
                box_map[location] = box_id
                stats['boxes_existing'] += 1
                logger.info(f"Found existing box: {location} ({box_id})")
            else:
                # Create new box
                new_box = {
                    'name': location,
                    'location': location.split(' - ')[0] if ' - ' in location else location,
                    'sublocation': location.split(' - ')[1] if ' - ' in location and len(location.split(' - ')) > 1 else None
                }
                ...
                logger.info(f"Created new box: {location} ({box_id})")
    ...
    async def _create_or_update_items(
        ...
        for row in parsed_rows:
            box_location = row.pop('box_location')
            box_id = box_map[box_location]
            ...
            item_data = {
                ...
                'box_id': str(box_id),  # Convert UUID to string for Supabase
            ...
            if response.data and len(response.data) > 0:
                ...
                logger.info(f"Updated item: {item_data['name']} in box {box_location}")
            else:
                # Create new item
                client.table('items').insert(item_data).execute()
                stats['items_created'] += 1
                logger.info(f"Created item: {item_data['name']} in box {box_location}")
    ...
    async def _generate_qr_codes(self, box_map: Dict[str, UUID], stats: Dict) -> List[Dict]:
        ...
        Returns:
            List of box info dicts with QR URLs
        """
        client = get_supabase_client()
        boxes_info = []

        for box_name, box_id in box_map.items():
            # Get box details
            ...
            if not qr_url:
                try:
                    qr_url = await self.qr_generator.generate_qr_for_box(str(box_id), box_name)
                    # Update box with QR URL (only if not placeholder)
                    if qr_url and 'placeholder' not in qr_url.lower():
                        client.table('boxes').update({'qr_code_url': qr_url}).eq('id', str(box_id)).execute()
                        logger.info(f"Generated QR code for box: {box_name}")
                    else:
                        logger.warning(f"QR code generation returned placeholder for box: {box_name}")
                        stats['warnings'].append(f"QR code not generated for {box_name} (storage bucket may not exist)")
                except Exception as e:
                    logger.warning(f"Failed to generate QR code for box {box_name}: {e}")
                    qr_url = None
                    stats['warnings'].append(f"QR code generation failed for {box_name}: {str(e)}")
